...
public class LinkedListIndexedCollection<E> implements List<E>{
	...
	private static class ElementsGetterList<T> implements ElementsGetter<T>{
		 ...
		 * @param node
		 */
		private ElementsGetterList(ListNode node, long savedModficationCount, LinkedListIndexedCollection<T> collection) {
			this.node = node;
	...
	public ElementsGetter<E> createElementsGetter() {
		return new ElementsGetterList<E>(first, modificationCount, this);
	}
