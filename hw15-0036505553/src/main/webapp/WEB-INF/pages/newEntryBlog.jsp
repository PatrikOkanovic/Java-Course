...
<body>
	...
	<c:when test="${not empty sessionScope['current.user.nick']}">
		<c:if test="${sessionScope['current.user.nick'].equals(author)}">
			<form action="<% out.write(request.getContextPath());%>/servleti/addNewEntryBlog" method="post">
				
				<div>
					<span class="formLabel">Title</span><input type="text" name="title"  size="20"/>
				</div>
				<div>
					<span class="formLabel">Text</span><input type="text" name="text"  size="50"/>
				</div>
